...
export const CardBar = forwardRef(({ open: inOpen, cardNo: inCardNo }: CardBarProp, ref: Ref<ICardBar>) => {
  ...
  function CardPanel() {
    return (
      <LocalizationProvider dateAdapter={AdapterDayjs}>
        <InlineEdit value={card!.objective} onSubmit={(target, value) => handleObjectiveChanged(value)} />
        ...
        <InlineEdit multiline value={card!.content} onSubmit={(target, value) => handleDescriptionChanged(value)} />
        ...
        <Stack direction="column">
          <Box>
            ...
            Deferred until:
            <DatePickButton
              value={card!.deferUntil ? new Date(card!.deferUntil.seconds * 1000) : null}
          ...
          <Box>
            Updated:
            {card?.updatedAt && new Date(card?.updatedAt.seconds * 1000).toLocaleString()}
          </Box>
          <Box>
            Completed at:
            {card!.completedAt ? new Date(card!.completedAt.seconds * 1000).toLocaleString() : 'N/A'}
          </Box>
        </Stack>
  ...
  return (
    <Drawer open={open} anchor="right" variant="persistent">
      ...
      <DrawerHeader />
      <DrawerHeader>
        <IconButton onClick={() => app.toast('refresh card: not implemented', 'warning')}>
      ...
      <Divider />
      <Box ref={cardRef} tabIndex={-1} sx={{ p: 1, width: 400, outline: '0px solid transparent' }}>
